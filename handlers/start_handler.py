...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.effective_message.reply_text("❌ Виникла помилка при виконанні команди /start.")
...
async def show_main_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.effective_message.reply_text("❌ Виникла помилка при відображенні меню.")
...
async def show_group_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.effective_message.reply_text("❌ Виникла помилка при відображенні меню у груповому чаті.")
...
async def latest_video_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if video_url:
            ...
            await update.effective_message.reply_text("⚠️ Наразі немає доступного відео в списку.")
            logger.warning("⚠️ Наразі немає доступного відео в списку.")
    except Exception as e:
        logger.error(f"❌ Помилка у виконанні команди /latest_video: {e}")
        await update.effective_message.reply_text("❌ Виникла помилка при отриманні відео.")
...
async def feedback_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not feedback_text and update.effective_message and update.effective_message.text:
        feedback_text = update.effective_message.text.replace('/feedback', '').strip()
    ...
    try:
        ...
        await update.effective_message.reply_text("✅ *Ваш відгук успішно надіслано. Дякуємо!*", parse_mode="Markdown")
        logger.info(f"✅ Відгук надіслано адміністратору: {feedback_text}")
    except Exception as e:
        logger.error(f"❌ Помилка при надсиланні відгуку: {e}")
        await update.effective_message.reply_text("❌ Виникла помилка при надсиланні відгуку. Спробуйте пізніше.")
